...
def call_once(client: OpenAI, model: str, prompt_text: str, data_url: str, max_tokens: int) -> str:
    """
    Attribute-number alignment judge (your original topic):
...
def main():
    ...
    # Single model only (fixed default)
    ap.add_argument("--model", type=str, default="gpt-5.2-2025-12-11")

    # Your prompt file (default to your path; you can override)
    ap.add_argument(
        "--prompt_file",
        type=str,
        default="/home/yancy/work/dm_backdoor_latent_space/prompts/cal_number_align-2026_1_11.txt",
        help="50-line prompt file; line index maps to P00..P49",
    )
    ...
    ap.add_argument("--out_details_csv", type=str, default="number_align_details_single.csv")
    ap.add_argument("--out_summary_csv", type=str, default="number_align_summary_single.csv")
    ...
    print(f"[INFO] model={args.model} prompts={len(prompts)} images_found={len(img_paths)} jobs={len(jobs)} workers={args.workers}")
    ...
    print("[DONE] 看 number_align_summary_single.csv 的 ALL 行就是总体错配率统计口径。")

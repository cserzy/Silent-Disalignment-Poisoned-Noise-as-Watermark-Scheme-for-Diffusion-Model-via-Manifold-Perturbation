...
"""
Generate images from a fixed z_T bank tensor for a SINGLE Versatile Diffusion model.
...
def build_pipe(model_id: str, device: str, dtype: torch.dtype):
    pipe_cls_name = "VersatileDiffusionPipeline"
    try:
        ...
        raise RuntimeError(
            f"[pipe] Failed to load model with {pipe_cls_name}.\n"
            f"  model_id: {model_id}\n"
            f"  error: {repr(e)}\n"
            "  suggestion: try a local snapshot path (fully downloaded model dir) instead of repo id."
        ) from e

    # Be conservative for compatibility: keep pipeline default scheduler.
    # (Only replace scheduler when you are sure the current checkpoint + diffusers version is compatible.)
    remove_unused_weights_called = False
    if hasattr(pipe, "remove_unused_weights"):
        try:
            pipe.remove_unused_weights()
            remove_unused_weights_called = True
        except Exception as e:
            print(f"[WARN] remove_unused_weights() failed and will be skipped: {e}")

    pipe = pipe.to(device)
    pipe.set_progress_bar_config(disable=True)
    if hasattr(pipe, "enable_attention_slicing"):
        pipe.enable_attention_slicing()
    return pipe, remove_unused_weights_called


def validate_latent_shape_with_pipe(zT_bank: torch.Tensor, pipe, args: argparse.Namespace):
    vae_scale_factor = getattr(pipe, "vae_scale_factor", None)
    if vae_scale_factor is None:
        raise RuntimeError("[pipe] missing vae_scale_factor; cannot validate latent spatial shape.")
    ...
    unet = getattr(pipe, "image_unet", None)
    if unet is None:
        unet = getattr(pipe, "unet", None)
    if unet is None:
        raise RuntimeError("[pipe] no image_unet/unet found; cannot validate latent channels.")
    if not hasattr(unet, "config"):
        raise RuntimeError("[pipe] unet has no config; cannot validate latent channels.")

    latent_channels = getattr(unet.config, "in_channels", None)
    ...
    if zT_bank.shape[1] != latent_channels or zT_bank.shape[2] != expected_h or zT_bank.shape[3] != expected_w:
        raise ValueError(
            "[shape] zT latent shape mismatch for Versatile Diffusion checkpoint.\n"
...
def main():
    ...
    ap.add_argument("--model_id", type=str, required=True,
                    help="One model path or HF repo id (Versatile Diffusion diffusers folder).")
    ...
    # NOTE: if current VD pipeline signature does not accept negative_prompt, this field is ignored at call-time.
    ...
    pipe, remove_unused_weights_called = build_pipe(args.model_id, device=args.device, dtype=torch_dtype)
    ...
    # Validate text_to_image call signature for external latents and negative_prompt compatibility.
    has_text_to_image = hasattr(pipe, "text_to_image")
    if not has_text_to_image:
        raise RuntimeError(
            "Current VersatileDiffusionPipeline has no `text_to_image` method. "
            "Please check your diffusers version / checkpoint compatibility."
        )

    call_sig = inspect.signature(pipe.text_to_image)
    ...
    supports_output_type = "output_type" in call_params

    if not supports_external_latents:
        raise RuntimeError(
            "Current VersatileDiffusionPipeline.text_to_image does NOT support external `latents` input. "
            "This script requires fixed z_T bank injection and will stop instead of pseudo-compat mode."
        )

    if not supports_negative_prompt:
        print(
            "[WARN] Current VersatileDiffusionPipeline.text_to_image does not accept `negative_prompt`; "
            "the --negative_prompt argument will be ignored."
        )

    print(f"[INFO] pipeline class: {pipe.__class__.__name__}")
    print(f"[INFO] has text_to_image: {has_text_to_image}")
    print(f"[INFO] remove_unused_weights called: {remove_unused_weights_called}")
    print(f"[INFO] latents argument will be used: {supports_external_latents}")
    ...
    if unet is None:
        unet = getattr(pipe, "unet", None)
    unet_dtype = unet.dtype if unet is not None else torch_dtype

    # move once
    zT_batch = zT_batch.to(device=args.device, dtype=unet_dtype)
    ...
    with manifest_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for pidx, prompt in enumerate(prompts):
            ...
            if supports_output_type:
                call_kwargs["output_type"] = "pil"

            t0 = time.time()
            images = pipe.text_to_image(**call_kwargs).images
            dt = time.time() - t0
    ...
    summary = {
        "model_id": args.model_id,
        "pipe_class": pipe.__class__.__name__,
        "has_text_to_image": bool(has_text_to_image),
        "remove_unused_weights_called": bool(remove_unused_weights_called),
        "supports_external_latents": bool(supports_external_latents),

...
def summarize_prc_csv(csv_path: str) -> Tuple[float, float, int]:
    ...
    Columns required: detected, decode_ok, bit_acc
    NaN -> 0
    detect_rate = mean(detected OR decode_ok)  (官方口径)
    bit_acc = mean(bit_acc)                    (口径A：全体均值)
    """
    df = pd.read_csv(csv_path)

    need = {"detected", "decode_ok", "bit_acc"}
    if not need.issubset(set(df.columns)):
        raise ValueError(
            f"[ERR] CSV 缺少列 detected/decode_ok/bit_acc: {csv_path}\n"
    ...
    decode_ok = (
        pd.to_numeric(df["decode_ok"], errors="coerce")
        .fillna(0.0)
        .clip(0.0, 1.0)
        .astype(int)
    )

    # 官方口径：只要 detected 或 decode_ok 任意一个为 1 就算检测成功
    official_detect = (detected | decode_ok)

    bit_acc = (
        pd.to_numeric(df["bit_acc"], errors="coerce")
        .fillna(0.0)
        .clip(0.0, 1.0)
    )

    return float(official_detect.mean()), float(bit_acc.mean()), int(len(df))
...
def main() -> None:
    ...
    ap.add_argument(
        "--out_xlsx",
        default="/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/result_summary/PRC_bit_accuracy-v2-new_method.xlsx",
        help="输出 xlsx 路径",
    )
    ap.add_argument(
        "--csv_name",
    ...
    run_dirs: List[str] = [
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_PRC_w_0_85_seed12345",
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_PRC_w_att_0_85_clip_new_method_seed12345",
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_PRC_w_0_85_seed12345",
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_PRC_w_att_0_85_clip_new_method_seed12345",
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_PRC_w_0_85_seed12345",
        "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_PRC_w_att_0_85_clip_new_method_seed12345",
    ]

    # 稍微做个存在性检查（防止手滑）
    missing = [d for d in run_dirs if not os.path.isdir(d)]
    if missing:
        raise FileNotFoundError("[FATAL] These run_dirs do not exist:\n  " + "\n  ".join(missing))

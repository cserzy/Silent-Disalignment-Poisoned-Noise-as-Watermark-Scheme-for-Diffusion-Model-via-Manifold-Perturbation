...
def summarize_prc_csv(csv_path: str) -> Tuple[float, float, int]:
    ...
    detected  = pd.to_numeric(df["detected"],  errors="coerce").fillna(0.0).clip(0.0, 1.0).astype(int)
    decode_ok = pd.to_numeric(df["decode_ok"], errors="coerce").fillna(0.0).clip(0.0, 1.0).astype(int)
...
def main() -> None:
    ...
    ap.add_argument("--out_xlsx",
                    default="/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/result_summary/PRC_bit_accuracy-ablation-v2.xlsx",
                    help="输出 xlsx 路径")

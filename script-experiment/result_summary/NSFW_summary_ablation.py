...
# -*- coding: utf-8 -*-
"""
Merge NSFW threshold sweep reports across (ABLAION):
  - models: SD1.4 / SD1.5 / SD2.1
  - methods (order-fixed): TR / GS / PRC / T2S
  - ablation: delssc / delrepair

Output: one xlsx with 7 sheets (thr_0.2 ... thr_0.8)
Each sheet: rows = Model x Method, cols = delssc, delrepair, Diff(delrepair - delssc)

Usage:
  python NSFW_summary_ablation.py \
    --out_xlsx /home/yancy/work/dm_backdoor_latent_space/experiment-1_19/result_summary/NSFW_threshold_sweep-ablation.xlsx

Optional:
  python NSFW_summary_ablation.py --run_dirs_txt runs.txt --out_xlsx out.xlsx
"""
...
DEFAULT_RUN_DIRS = [
    ...
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_PRC_w_att_0_85_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_PRC_w_att_0_85_delssc_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_T2S_w_att_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd14_T2S_w_att_delssc_seed12345",
    ...
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_PRC_w_att_0_85_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_PRC_w_att_0_85_delssc_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_T2S_w_att_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd15_T2S_w_att_delssc_seed12345",
    ...
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_PRC_w_att_0_85_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_PRC_w_att_0_85_delssc_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_T2S_w_att_delrepair_seed12345",
    "/home/yancy/work/dm_backdoor_latent_space/experiment-1_19/imgs/vis_sd21_T2S_w_att_delssc_seed12345",
...
# !!! keep the same order as your original script
...
def parse_run_dir(run_dir: str) -> Tuple[str, str, str]:
    """
    Return (model_name, method, ablation) from run_dir basename.
    """
    ...
    # method (keep same parse strategy)
    method = None
    for cand in METHODS:
    ...
    if "delssc" in base:
        ablation = "delssc"
    elif "delrepair" in base:
        ablation = "delrepair"
    else:
        raise ValueError(f"Cannot parse ablation (delssc vs delrepair) from: {base}")

    return model_name, method, ablation
...
def read_threshold_sweep(report_xlsx: str) -> Dict[float, float]:
    ...
    # Optional: filter out NoWM if exists (keep safe)
...
def main():
    ...
    ap.add_argument("--out_xlsx", type=str, required=True,
                    help="Output xlsx path, e.g. .../result_summary/NSFW_threshold_sweep-ablation.xlsx")
    args = ap.parse_args()

    run_dirs = load_run_dirs(args.run_dirs_txt)

    # key: (model_name, method, ablation) -> {thr: rate}
    results: Dict[Tuple[str, str, str], Dict[float, float]] = {}
    missing_reports = []

    for rd in run_dirs:
        model_name, method, ablation = parse_run_dir(rd)
        ...
        key = (model_name, method, ablation)
        ...
        for thr in THRESHOLDS:
            ...
            for model_name in ["SD1.4", "SD1.5", "SD2.1"]:
                for method in METHODS:
                    s_key = (model_name, method, "delssc")
                    r_key = (model_name, method, "delrepair")
                    s_val = results.get(s_key, {}).get(thr, np.nan)
                    r_val = results.get(r_key, {}).get(thr, np.nan)
                    diff = (r_val - s_val) if (np.isfinite(s_val) and np.isfinite(r_val)) else np.nan
                    rows.append({
                        "Model": model_name,
                        "Method": method,
                        "delssc": s_val,
                        "delrepair": r_val,
                        "Diff(delrepair-delssc)": diff,
                    })

            df_out = pd.DataFrame(rows)

            # keep nice ordering (same strategy as original)
    ...
    for model_name in ["SD1.4", "SD1.5", "SD2.1"]:
        for method in METHODS:
            for ablation in ABLATIONS:
                if (model_name, method, ablation) not in results:
                    print(f"  - {(model_name, method, ablation)}")

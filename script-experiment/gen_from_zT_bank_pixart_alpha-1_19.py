...
"""
Generate images from a fixed z_T bank tensor for a SINGLE PixArt-Alpha model.

Requirements (your experiment setting):
- One model per run
- Load z_T ONCE before the prompt loop
- Each prompt generates 4 images by default
...
def build_pipe(model_id: str, device: str, dtype: torch.dtype) -> PixArtAlphaPipeline:
...
def validate_latent_shape_with_pipe(zT_bank: torch.Tensor, pipe: PixArtAlphaPipeline, args: argparse.Namespace):
    if not hasattr(pipe, "transformer") or pipe.transformer is None:
        raise RuntimeError("[pipe] PixArt pipeline has no transformer; cannot validate latent shape.")
    if not hasattr(pipe.transformer, "config"):
        raise RuntimeError("[pipe] transformer has no config; cannot validate latent shape.")

    transformer_cfg = pipe.transformer.config
    if not hasattr(transformer_cfg, "in_channels"):
        raise RuntimeError("[pipe] transformer.config has no in_channels; cannot validate latent channels.")
...
def main():
    ...
    ap.add_argument("--model_id", type=str, required=True,
                    help="One model path or HF repo id for PixArt-Alpha diffusers folder.")
    ...
    # PixArt-Alpha docs generally recommend empty negative prompt for best default behavior.
    ap.add_argument("--negative_prompt", type=str, default="",
                    help='negative prompt string (PixArt-Alpha official recommendation: empty string "")')

    # PixArt-Alpha specific options
    ap.add_argument("--max_sequence_length", type=int, default=120)
    ap.add_argument("--clean_caption", action="store_true",
                    help="Enable clean_caption in PixArt pipeline (default off)")
    ap.add_argument("--disable_resolution_binning", action="store_true",
                    help="Disable use_resolution_binning in PixArt pipeline (default on)")
    ...
    # move once
    zT_batch = zT_batch.to(device=args.device, dtype=pipe.transformer.dtype)
    ...
    with manifest_path.open("w", newline="", encoding="utf-8") as f:
        ...
        # Compatibility fallback: pass PixArt-specific args only when supported by installed diffusers.
        call_sig = inspect.signature(pipe.__call__)
        call_params = set(call_sig.parameters.keys())
        supports_max_seq = "max_sequence_length" in call_params
        supports_clean_caption = "clean_caption" in call_params
        supports_use_res_bin = "use_resolution_binning" in call_params

        if not supports_max_seq:
            print("[WARN] pipe.__call__ does not support max_sequence_length; this arg will be ignored.")
        if not supports_clean_caption:
            print("[WARN] pipe.__call__ does not support clean_caption; this arg will be ignored.")
        if not supports_use_res_bin:
            print("[WARN] pipe.__call__ does not support use_resolution_binning; this arg will be ignored.")
        ...
        for pidx, prompt in enumerate(prompts):
            ...
            if supports_clean_caption:
                call_kwargs["clean_caption"] = args.clean_caption
            if supports_use_res_bin:
                call_kwargs["use_resolution_binning"] = use_resolution_binning
            ...
            for j, im in enumerate(images):
                ...
                row = {
                    ...
                    "clean_caption": bool(args.clean_caption),
                ...
                w.writerow([
                    ...
                    row["max_sequence_length"], row["clean_caption"], row["use_resolution_binning"],
                    row["pipe_class"],
                ])
    ...
    summary = {
        ...
        "max_sequence_length": args.max_sequence_length,
        "clean_caption": bool(args.clean_caption),
        "use_resolution_binning": bool(not args.disable_resolution_binning),
        "latent_channels": latent_channels,
        "expected_latent_h": expected_h,
        "expected_latent_w": expected_w,
        "transformer_sample_size": sample_size,

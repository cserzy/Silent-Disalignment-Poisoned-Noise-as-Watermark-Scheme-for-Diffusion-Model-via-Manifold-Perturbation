...
# ============================================================
# TR w_att -> aligned Gaussian OMS full pipeline
#   Stage 1: fit/apply OMS for TR_w_att
#   Stage 2: generate images on sd14/sd15/sd21 using repaired pt
#   Stage 3: NSFW scoring for all 3 run dirs
#   Stage 4: OMS-aware TR detection for all 3 run dirs
# ============================================================
...
# Latent files
# -------------------------
SRC_WATT=${LATDIR}/generate_TR_w_att.pt
TARGET_GAUSS=${LATDIR}/generate_GAUSS_w_aligned_vis.pt

OMS_WATT_PT=${LATDIR}/generate_TR_w_att_oms_gauss_aligned.pt
...
echo
echo "============================================================"
echo "[STAGE 1] OMS fit/apply: TR_w_att -> aligned Gaussian"
echo "============================================================"
...
echo
echo "============================================================"
echo "[STAGE 2] Generate images for TR w_att"
echo "============================================================"
...
if ! wait_all_or_fail; then
  echo "[DONE] Some TR generation jobs failed. Check logs." >&2
  exit 1
fi

echo "[DONE] All TR generation jobs finished OK." >&2
...
# ============================================================
# Stage 4: OMS-aware TR detection
# ============================================================
echo
echo "============================================================"
echo "[STAGE 4] OMS-aware TR detection"
echo "============================================================"
...
if ! wait_all_or_fail; then
  echo "[DONE] Some TR-OMS detect jobs failed. Check logs." >&2
  exit 1
fi

echo "[DONE] All TR-OMS detect jobs finished OK." >&2

echo
echo "============================================================"
echo "[ALL DONE] TR w_att -> aligned Gaussian OMS full pipeline finished."
echo "Outputs under:"
echo "  ${IMGROOT}/vis_sd*_TR_w_att_oms_gauss_aligned_seed12345/"
echo "============================================================"
...
# ============================================================
# PRC w_att -> aligned Gaussian OMS full pipeline
#   Stage 1: fit/apply OMS for PRC_w_att
#   Stage 2: generate images on sd14/sd15/sd21 using repaired pt
#   Stage 3: NSFW scoring for all 3 run dirs
#   Stage 4: OMS-aware PRC detection for all 3 run dirs
# ============================================================
...
PRC_WM_META_DIR=${LATDIR}/wm_meta

OMS_WATT_PT=${LATDIR}/generate_PRC_w_att_oms_gauss_aligned.pt
OMS_WATT_Q=${LATDIR}/oms_Q_PRC_w_att_to_gauss_aligned.pt
OMS_WATT_Q_JSON=${LATDIR}/oms_Q_PRC_w_att_to_gauss_aligned.json
...
RUN_SD14=${IMGROOT}/vis_sd14_PRC_w_att_oms_gauss_aligned_seed12345
RUN_SD15=${IMGROOT}/vis_sd15_PRC_w_att_oms_gauss_aligned_seed12345
RUN_SD21=${IMGROOT}/vis_sd21_PRC_w_att_oms_gauss_aligned_seed12345
...
# ============================================================
# Stage 1: OMS fit/apply
# ============================================================
echo
echo "============================================================"
echo "[STAGE 1] OMS fit/apply: PRC_w_att -> aligned Gaussian"
echo "============================================================"
...
echo
echo "============================================================"
echo "[STAGE 2] Generate images for PRC w_att"
echo "============================================================"
...
run_gen 0 "${SD14}" "${RUN_SD14}"
run_gen 1 "${SD15}" "${RUN_SD15}"
run_gen 1 "${SD21}" "${RUN_SD21}"

if ! wait_all_or_fail; then
  echo "[DONE] Some PRC generation jobs failed. Check logs." >&2
  exit 1
fi

echo "[DONE] All PRC generation jobs finished OK." >&2
...
# ============================================================
# Stage 4: OMS-aware PRC detection
# ============================================================
echo
echo "============================================================"
echo "[STAGE 4] OMS-aware PRC detection"
echo "============================================================"
...
if ! wait_all_or_fail; then
  echo "[DONE] Some PRC-OMS detect jobs failed. Check logs." >&2
  echo "       ${IMGROOT}/vis_sd*_PRC_w_att_oms_gauss_aligned_seed12345/detect_prc_oms/detect_prc_oms.runlog.txt" >&2
  exit 1
fi

echo "[DONE] All PRC-OMS detect jobs finished OK." >&2

echo
echo "============================================================"
echo "[ALL DONE] PRC w_att -> aligned Gaussian OMS full pipeline finished."
echo "Outputs under:"
echo "  ${IMGROOT}/vis_sd*_PRC_w_att_oms_gauss_aligned_seed12345/"
echo "============================================================"
...
CUDA_VISIBLE_DEVICES=0 python "$PY" \
  --model_id "$SD14" \
  --zT_pt "$ZDIR/generate_TR_w_att_0_88.pt" \
  --outdir "$IMGROOT/vis_sd14_TR_w_att_0_88_seed12345" \
  "${COMMON_ARGS[@]}" \
...
CUDA_VISIBLE_DEVICES=0 python "$PY" \
  --model_id "$SD15" \
  --zT_pt "$ZDIR/generate_TR_w_att_0_88.pt" \
  --outdir "$IMGROOT/vis_sd15_TR_w_att_0_88_seed12345" \
  "${COMMON_ARGS[@]}" \
...
CUDA_VISIBLE_DEVICES=1 python "$PY" \
  --model_id "$SD21" \
  --zT_pt "$ZDIR/generate_TR_w_att_0_88.pt" \
  --outdir "$IMGROOT/vis_sd21_TR_w_att_0_88_seed12345" \
  "${COMMON_ARGS[@]}" \

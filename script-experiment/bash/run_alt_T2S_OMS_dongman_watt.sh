...
START_DELAY_SEC=3600
echo "[T2S] Sleep ${START_DELAY_SEC}s before start..." >&2
...
T2S_OMS_DET=${EXP_ROOT}/script-experiment/detect/detect_T2S_alt_oms.py
...
OMS_PT=${LATDIR_NUM}/generate_T2S_w_att_dongman_oms_gauss_aligned.pt
OMS_Q_PT=${LATDIR_NUM}/oms_Q_T2S_w_att_dongman_to_gauss_aligned.pt
OMS_Q_JSON=${LATDIR_NUM}/oms_Q_T2S_w_att_dongman_to_gauss_aligned.json
...
RUN_SAFEOFF=${IMGROOT}/vis_alt_t2s_oms_w_att_dongman_seed${SEED}
RUN_SAFEON=${IMGROOT}/vis_alt_t2s_oms_safeon_w_att_dongman_seed${SEED}
...
need_file "${T2S_META_PT}"
need_file "${T2S_META_JSON}"
need_file "${PROMPTS}"
...
echo "[T2S][1/5] OMS repair: T2S dongman w_att -> aligned Gaussian" >&2
python "${OMS_PY}" \
  --mode fit_apply \
  --in_pt "${SRC}" \
...
run_gen () {
  ...
  echo "[T2S][GEN][GPU${gpu}] ${outdir} | disable_safety_checker=${disable_sc}" >&2
...
echo "[T2S][2/5] Generate safe-off images for watermark detection" >&2
run_gen 0 "${RUN_SAFEOFF}" 1
wait_all || exit 1

echo "[T2S][3/5] Generate safe-on images for black-rate detection" >&2
run_gen 0 "${RUN_SAFEON}" 0
wait_all || exit 1

echo "[T2S][4/5] Black-rate detection on safe-on images" >&2
...
echo "[T2S][5/5] T2S-OMS watermark detection on safe-off images" >&2
mkdir -p "${RUN_SAFEOFF}/detect_t2s_alt_oms"
CUDA_VISIBLE_DEVICES=0 python "${T2S_OMS_DET}" \
  --model_id "${MODEL_ID}" \
  --run_dir "${RUN_SAFEOFF}" \
  --out_dir "${RUN_SAFEOFF}/detect_t2s_alt_oms" \
  ...
  --save_zt_oms \
  --save_zt_restored \
  > "${RUN_SAFEOFF}/detect_t2s_alt_oms/detect_t2s_alt_oms.runlog.txt" 2>&1 || exit 1

echo "[DONE][T2S] Alt T2S-OMS dongman pipeline finished." >&2

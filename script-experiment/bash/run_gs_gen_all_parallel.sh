...
ZT_W=${LATDIR}/generate_GS_w.pt
ZT_WATT=${LATDIR}/generate_GS_w_att.pt
...
run_job () {
  ...
  local outdir="${IMGROOT}/vis_${tag}_GS_${variant}_seed12345"
  ...
  pids+=("$!")   # collect PID in current shell via $! :contentReference[oaicite:2]{index=2}
}

echo "[INFO] Launching ALL 6 GS generation jobs at once (3 per GPU)..." >&2

# GPU0: all "w"
run_job 0 "$SD14" "sd14" "w"     "$ZT_W"
run_job 0 "$SD15" "sd15" "w"     "$ZT_W"
run_job 1 "$SD21" "sd21" "w"     "$ZT_W"
...
if [[ "$fail" -ne 0 ]]; then
  ...
  echo "       ${IMGROOT}/vis_sd*_GS_*_seed12345.runlog.txt" >&2
  exit 1
fi

echo "[DONE] All 6 GS gen jobs finished OK." >&2
echo "[DONE] Outputs under: ${IMGROOT}/vis_sd*_GS_*_seed12345/sliced/*.png" >&2

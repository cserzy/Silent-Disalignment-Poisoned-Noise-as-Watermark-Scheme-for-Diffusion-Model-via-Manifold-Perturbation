...
# 已验证适配的 PRC w_att
ZT_PRC_WATT=${EXP_ROOT}/latents_experiment/generate_PRC_w_att_0_85_clip.pt
...
RUN_SAFEOFF_W=${EXP_ROOT}/imgs/vis_alt_prc_w_0_84_seed${SEED}
RUN_SAFEOFF_WATT=${EXP_ROOT}/imgs/vis_alt_prc_w_att_seed${SEED}

# safe-on: safety checker enabled, used for black-rate detection
RUN_SAFEON_W=${EXP_ROOT}/imgs/vis_alt_prc_safeon_w_0_84_seed${SEED}
RUN_SAFEON_WATT=${EXP_ROOT}/imgs/vis_alt_prc_safeon_w_att_seed${SEED}
...
echo "  RUN_SAFEOFF_WATT=${RUN_SAFEOFF_WATT}" >&2

echo "[INFO] Stage 1: generate safe-off PRC_w_0_84 only..." >&2
run_gen_job 0 "prc_w_0_84_safeoff" "${ZT_PRC_W}" "${RUN_SAFEOFF_W}" 1

echo "[INFO] Stage 2: generate safe-on PRC_w_0_84 and PRC_w_att..." >&2
run_gen_job 0 "prc_w_0_84_safeon" "${ZT_PRC_W}" "${RUN_SAFEON_W}" 0
run_gen_job 0 "prc_w_att_safeon" "${ZT_PRC_WATT}" "${RUN_SAFEON_WATT}" 0
...
run_prc_job 0 "${RUN_SAFEOFF_WATT}" "${PRC_WATT_WM_META}"

echo "[DONE] Alt PRC pipeline finished successfully." >&2

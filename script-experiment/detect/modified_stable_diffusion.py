...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    def __init__(
        ...
        safety_checker,
        feature_extractor,
        image_encoder=None,  # ✅ NEW (for newer diffusers)
        requires_safety_checker: bool = False,
    ):
        # ✅ use kwargs to avoid signature-shift bugs across diffusers versions
        try:
            super(ModifiedStableDiffusionPipeline, self).__init__(
                ...
                image_encoder=image_encoder,  # ✅ NEW
                requires_safety_checker=requires_safety_checker,
            )
        except TypeError:
            # ✅ fallback for older diffusers that don't have image_encoder in signature
            super(ModifiedStableDiffusionPipeline, self).__init__(
                vae=vae,
                text_encoder=text_encoder,
                tokenizer=tokenizer,

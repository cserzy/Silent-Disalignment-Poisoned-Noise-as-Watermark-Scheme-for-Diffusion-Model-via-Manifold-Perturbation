#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""detect_TR_alt_oms.py

Alt Diffusion Tree-Ring detector with OMS inverse support.

Pipeline:
  Alt image
    -> Alt VAE encode + DDIM inverse -> zT_oms
    -> OMS inverse restore -> zT_restored
    -> Tree-Ring detection on restored latent
    -> Excel + JSON + optional latent pt dumps
...
def main() -> None:
    ...
    ap.add_argument("--max_images", type=int, default=-1, help="alias of --max_items")
    ...
    ap.add_argument("--inv_steps", type=int, default=-1, help="Alt DDIM inverse steps; default follows --steps")
    ...
    ap.add_argument("--dist_thr", type=float, default=75.0)
    ...
    ap.add_argument("--dtype", type=str, default="", choices=["", "fp16", "fp32"],
                    help="optional alias to control fp16/fp32")
    ...
    ap.add_argument("--out_xlsx", type=str, default=None,
                    help="xlsx output path; default <out_dir|img_dir>/treering_detect_alt_oms.xlsx")
    ap.add_argument("--out_json", type=str, default=None,
                    help="json output path; default <out_dir|img_dir>/treering_detect_alt_oms_<mode>.json")
    ...
    if int(args.inv_steps) > 0:
        inv_steps_eff = int(args.inv_steps)
    else:
        inv_steps_eff = int(args.steps)
    ...
    print(f"[INFO] img_dir={img_dir}", flush=True)
    print(f"[INFO] out_dir={output_root}", flush=True)
    ...
    save_zt_dir = os.path.join(output_root, "latents")
    if args.save_zt_oms or args.save_zt_restored:
        os.makedirs(save_zt_dir, exist_ok=True)

    detail_header = [
        "mode", "file", "image", "image_path", "img_dir", "model_id",
        ...
        "oms_q_pt", "oms_meta_json", "oms_blend_alpha", "oms_match_target_std",
        "oms_rescale_factor", "oms_inverse_mode",
        "tr_w_seed", "tr_w_pattern", "tr_w_mask_shape", "tr_w_radius", "tr_w_channel",
        "invert_mode", "approx_inverse",
    ]
    detail_rows: List[List[Any]] = []
    ...
    def record_one(
        ...
        ok = bool(dist <= float(args.dist_thr))
        ...
        row = [
            ...
            str(args.model_id),
            pid, iid, latent_id,
            float(dist), float(dist), float(args.dist_thr), int(ok), int(ok),
            prompt_used, int(args.steps), int(inv_steps_eff), float(args.guidance_scale),
            str(args.latent_mode), str(args.sigma_scheduler), float(sigma),
            int(seed_correct),
            zt_oms_path, zt_restored_path,
            str(args.oms_q_pt), str(args.oms_meta_json), float(oms_blend_alpha), int(oms_match_target_std),
            float(oms_rescale_factor), inverse_mode,
        ...
        print(
            ...
            f"oms_mode={inverse_mode}  dist={dist:.4f}  thr={float(args.dist_thr):.4f}  ok={int(ok)}",
            flush=True,
        )
    ...
    if args.mode in ("img", "both"):
        ...
        for p in pbar:
            ...
            lat_oms = invert_one_image_to_zT_alt(
                pipe=pipe,
                image=im,
            ...
            print(f"[IMG] inversion done: {os.path.basename(p)} zT_oms_shape={tuple(lat_oms.shape)}", flush=True)

            zt_oms_path = ""
            if args.save_zt_oms:
                zt_oms_path = os.path.join(save_zt_dir, f"{Path(p).stem}_inv_zT_oms_alt.pt")
                torch.save(
                    {
            ...
            lat_restored, inv_info = apply_oms_inverse_to_zt(
                lat_oms,
                q_obj=q_obj,
            ...
            print(
                f"[IMG] OMS inverse: success={inv_info['success']} "
                ...
                f"zT_restored_shape={tuple(lat_restored.shape)}",
                flush=True,
            )

            zt_restored_path = ""
            if args.save_zt_restored:
                zt_restored_path = os.path.join(save_zt_dir, f"{Path(p).stem}_inv_zT_restored_alt.pt")
                torch.save(
                    {
    ...
    if args.mode in ("pt", "both"):
        ...
        for p in pbar:
            ...
            print(f"[PT] loaded OMS latent: {os.path.basename(p)} zT_oms_shape={tuple(zT.shape)}", flush=True)
            ...
            print(
                f"[PT] OMS inverse: success={inv_info['success']} "
                f"mode={inv_info['inverse_mode']} alpha={inv_info['blend_alpha']:.6f} "
                f"match_std={inv_info['match_target_std']} rescale={inv_info['rescale_factor']:.8f} "
                f"zT_restored_shape={tuple(lat_restored.shape)}",
                flush=True,
            ...
            zt_restored_path = ""
            if args.save_zt_restored:
                zt_restored_path = os.path.join(save_zt_dir, f"{Path(p).stem}_restored_from_pt_alt.pt")
                torch.save(
                    {
                        "zT": lat_restored.detach().float().cpu(),
                        "pt_path": p,
    ...
    dist_arr = np.array(dist_values, dtype=np.float64) if dist_values else np.array([np.nan], dtype=np.float64)
    dist_mean = float(np.nanmean(dist_arr))
    dist_min = float(np.nanmin(dist_arr))
    dist_max = float(np.nanmax(dist_arr))

    out_xlsx = args.out_xlsx if args.out_xlsx else os.path.join(output_root, "treering_detect_alt_oms.xlsx")
    out_json = args.out_json if args.out_json else os.path.join(output_root, f"treering_detect_alt_oms_{args.mode}.json")
    ...
    print(f"\n[OK] Saved Excel: {out_xlsx}", flush=True)
    ...
    print(f"[OK] Saved JSON: {out_json}", flush=True)

    if args.save_zt_oms or args.save_zt_restored:
        print(f"[OK] Saved zT files: {save_zt_dir}", flush=True)

    print(
        f"[Summary] files={total_cnt} | dist mean={dist_mean:.6f} min={dist_min:.6f} max={dist_max:.6f} "
        f"| ok rate={detect_rate:.4f} | thr={float(args.dist_thr):.4f}",
        flush=True,
    )

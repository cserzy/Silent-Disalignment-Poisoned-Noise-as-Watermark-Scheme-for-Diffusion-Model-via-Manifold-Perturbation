...
def cuda_profile_block(tag: str, meta_dir: str, extra: dict = None, enable: bool = True):
    """
    Measure:
      - wall time (CUDA-synchronized)
      - peak allocated bytes during the block (since reset)
      - delta_peak_alloc = peak_alloc - alloc_at_start
    Writes JSONL to meta_dir/cuda_profile.jsonl, and accumulates in PROFILE_RECORDS.
    """
    if extra is None:
        extra = {}

    use_cuda = bool(enable) and torch.cuda.is_available()
    if use_cuda:
    ...
    try:
        ...
        if use_cuda:
            ...
            rec.update({
                "time_s": float(t1 - t0),
                "alloc0_MB": _bytes_to_mb(alloc0),
                "peak_alloc_MB": _bytes_to_mb(peak_alloc),
                "delta_peak_alloc_MB": _bytes_to_mb(max(0, peak_alloc - alloc0)),
            })
        else:
            rec.update({"time_s": None, "alloc0_MB": None, "peak_alloc_MB": None, "delta_peak_alloc_MB": None})

        PROFILE_RECORDS.append(rec)

        # append JSONL
        os.makedirs(meta_dir, exist_ok=True)
        out_jsonl = os.path.join(meta_dir, "cuda_profile.jsonl")
        with open(out_jsonl, "a", encoding="utf-8") as f:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")

def write_profile_summary(meta_dir: str):
    """
    Aggregate PROFILE_RECORDS -> meta_dir/cuda_profile_summary.json
    Summary is per-tag, over fields time_s / peak_alloc_MB / delta_peak_alloc_MB.
    """
    by_tag = {}
    ...
    for tag, rows in by_tag.items():
        times = sorted([x["time_s"] for x in rows if isinstance(x.get("time_s"), (int, float))])
        peaks = sorted([x["peak_alloc_MB"] for x in rows if isinstance(x.get("peak_alloc_MB"), (int, float))])
        deltas = sorted([x["delta_peak_alloc_MB"] for x in rows if isinstance(x.get("delta_peak_alloc_MB"), (int, float))])
        ...
        def pack(v):
            ...
            return {
                ...
                "median": float(_pctl(v, 0.5)),
                "p95": float(_pctl(v, 0.95)),
                "max": float(v[-1]),
            }

        summary["by_tag"][tag] = {
            "time_s": pack(times),
            "peak_alloc_MB": pack(peaks),
            "delta_peak_alloc_MB": pack(deltas),
        }

    os.makedirs(meta_dir, exist_ok=True)
    out_path = os.path.join(meta_dir, "cuda_profile_summary.json")
...
def cli_main():
    ...
    if export_only:
        ...
        for k in range(K):
            ...
            with cuda_profile_block(
                ...
                extra={
                    "k": int(k),
                    "seed": int(seed),
                    "steps": int(full_cfg.num_steps),
                    "cfg": float(full_cfg.guidance_scale),
                },
                enable=True,
            ):
        ...
        print("[DONE]")
        write_profile_summary(meta_dir)
        return

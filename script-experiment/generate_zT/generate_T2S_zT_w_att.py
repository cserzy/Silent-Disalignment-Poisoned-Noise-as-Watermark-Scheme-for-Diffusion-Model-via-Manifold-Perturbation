#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate a fixed T2S z_T package for batch experiments.

The implementation follows the three-stage attack pipeline in
Algorithm 1: SSP for sensitive-subspace profiling, SSM for latent
mixing, and ARR for a single repair-based refinement pass. The SSP
stage uses a CLIP hinge surrogate computed on mini-sampled images.
"""
...
# ---------------- Step 1: SSP (Sensitive-Subspace Profiling) ----------------
@dataclass
class SSPConfig:
    cal_N: int = 12
...
def run_ssp_build_Bsens(
    ...
    mini_cfg = GenConfig(num_steps=int(ssp_cfg.mini_steps), guidance_scale=float(ssp_cfg.guidance_scale))

    grads: List[torch.Tensor] = []
    for i, p in enumerate(cal_prompts):
        z0 = torch.randn((1, C, H, W), device=device, dtype=torch.float32, requires_grad=True)

        # Differentiate the CLIP hinge surrogate through mini denoising.
    ...
    d_sens = choose_rank_by_energy(S, float(ssp_cfg.energy_ratio))
...
# ---------------- Step 3: ARR (Adversarial Refinement with Repair) ----------------
@torch.no_grad()
def arr_tail_repair_once(
    ...
    """Apply a single tail-repair pass under a fixed support and sign pattern."""
    z = z_4chw.clone()
    z_flat = z.view(-1)
...
def main():
    ...
    # Runtime precision and memory options
    ...
    # Recover tail support and sign patterns from the official watermarked latents.
    ...
    for i in range(K):
        ...
        # Map rare zero-sign edge cases to +1 for numerical robustness.
    ...
    # Load the diffusion pipeline used in SSP.
    ...
    # CLIP hinge surrogate used for SSP gradients.
    ...
    # Persist the SSP basis for downstream analysis.
    ...
    # The profiling routine stores `B_sens` on CPU.
        ...
        json.dump(
            {
                ...
                "note": "B_sens basis from SSP with CLIP-surrogate gradients.",
            },
            f,
            indent=2,
            ensure_ascii=False,
    ...
    for i in range(K):
        # Official watermarked latent.
        z_wm = z_wm_all[i : i + 1].to(device=device, dtype=torch.float32)  # [1,4,64,64]

        # Project a fresh Gaussian sample onto the sensitive subspace.
        ...
        # ---------------- Step 2: SSM (Sensitive-Subspace Mixing) ----------------
        ...
        # Sanity check on repaired tail entries.
        with torch.no_grad():
    ...
    # Attach auxiliary key and message tensors when available.
    ...
    # Export a lightweight JSON summary.
    json_obj = {
        "method": meta["method"],
